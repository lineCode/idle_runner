                       ...
                       UINT uBalloonTimeout /*=10*/)
{
    ...
    m_hWnd = ::CreateWindow(TRAYICON_CLASS, "", WS_POPUP, 
...
//            ASSERT(AfxIsValidString(szBalloonTitle));
        ...
        strncpy(m_tnd.szInfo, szBalloonTip, 255);
        if (szBalloonTitle)
            strncpy(m_tnd.szInfoTitle, szBalloonTitle, 63);
        else
...
BOOL CSystemTray::SetTooltipText(LPCTSTR pszTip)
{
    ...
    strncpy(m_tnd.szTip, pszTip, m_nMaxTooltipLength-1);
...
//        ASSERT(AfxIsValidString( szTitle));
    ...
    m_tnd.uFlags = NIF_INFO;
    strncpy(m_tnd.szInfo, szText, 256);
    if (szTitle)
        strncpy(m_tnd.szInfoTitle, szTitle, 64);
    else
        m_tnd.szInfoTitle[0] = '\0';
...
BOOL CALLBACK FindTrayWnd(HWND hwnd, LPARAM lParam)
{
    ...
    if (strcmp(szClassName, "TrayClockWClass") == 0)
    {
        LPRECT lpRect = (LPRECT) lParam;
        RECT rectClock;
...
BOOL CSystemTray::RemoveTaskbarIcon(HWND hWnd)
{
    ...
    {
		m_hWndInvisible = CreateWindowEx(0, "Static", "", WS_POPUP,
